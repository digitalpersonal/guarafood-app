                ...
                >
                    ...
                    <div className="overflow-y-auto space-y-6 pr-2 -mr-2">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                            <div>
                                <p className="font-bold text-lg text-gray-900">Pedido #{order.id.substring(0, 6)}</p>
                                <p className="text-sm text-gray-500">{new Date(order.timestamp).toLocaleString('pt-BR')}</p>
                            </div>
                                ...
                                <p className="text-sm text-gray-600">Telefone: {order.customerPhone}</p>
                                 {order.customerAddress && (
                                    <p className="text-sm text-gray-600 mt-1">
                                        <strong>Endereço:</strong> {order.customerAddress.street}, {order.customerAddress.number} - {order.customerAddress.neighborhood}
                                        {order.customerAddress.complement && `, ${order.customerAddress.complement}`}
                                        <br/>
                                        CEP: {order.customerAddress.zipCode}
                                    </p>
                                )}
                                <p className="text-sm text-gray-600">Pagamento: <span className="font-medium">{order.paymentMethod}</span></p>
                            </div>
                            <div className="space-y-1">
                                ...
                                <p className="text-sm text-gray-600">Telefone: {order.restaurantPhone}</p>
                            </div>
                        </div>
