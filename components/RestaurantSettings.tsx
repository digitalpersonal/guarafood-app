...
import type { Restaurant, OperatingHours, Order, WhatsAppConnection } from '../types';
...
const RestaurantSettings: React.FC<{ restaurantIdOverride?: number, onBack?: () => void, initialSection?: 'general' | 'whatsapp' }> = ({ restaurantIdOverride, onBack, initialSection = 'general' }) => {
    const { currentUser } = useAuth();
    const { addToast, confirm } = useNotification();
    const [restaurant, setRestaurant] = useState<Restaurant | null>(null);
    const [activeSection, setActiveSection] = useState<'general' | 'whatsapp'>(initialSection);
    ...
    const handleUpdateWhatsAppConnection = useCallback((conn: WhatsAppConnection | null) => {
        setRestaurant(prev => {
            if (!prev) return null;
            if (prev.whatsappConnection === conn) return prev;
            return { ...prev, whatsappConnection: conn };
        });
    }, []);
    ...
    return (
        <main className="p-4 max-w-2xl mx-auto space-y-6 pb-32">
            ...
            {/* SELETOR DE SUB-SEÇÃO: CONFIGURAÇÕES → GERAL / WHATSAPP */}
            <div className="flex items-center space-x-2 p-1.5 bg-gray-100/80 rounded-2xl border border-gray-200 shadow-inner">
                <button
                    type="button"
            ...
            {activeSection === 'whatsapp' ? (
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 animate-in fade-in duration-200">
                    <WhatsAppSettings 
                        restaurant={restaurant} 
                        onUpdateConnection={handleUpdateWhatsAppConnection} 
                    />
                </div>
            ) : (
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 animate-in fade-in duration-200">
                <h2 className="text-xl font-black text-gray-800 border-b pb-4 mb-6 uppercase tracking-widest">Painel de Configuração</h2>

                {/* BANNER DE ATALHO PARA WHATSAPP */}
                <div 
                    onClick={() => setActiveSection('whatsapp')}
                    className="mb-8 p-4 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-emerald-500/5 to-transparent border border-emerald-200 hover:border-emerald-300 transition-all cursor-pointer flex items-center justify-between group shadow-sm hover:shadow"
                >
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#25D366] text-white flex items-center justify-center shadow-md shadow-emerald-500/20 group-hover:scale-105 transition-transform">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h4 className="font-black text-sm text-gray-900">WhatsApp (API Oficial Meta)</h4>
                                <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md">Novo</span>
                            </div>
                            <p className="text-xs text-gray-500 font-medium">Conecte o WhatsApp do seu restaurante com a Meta Cloud API</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-1 font-black text-xs text-[#25D366] group-hover:translate-x-1 transition-transform">
                        <span>Acessar</span>
                        <span>→</span>
                    </div>
                </div>
