...
const Toast: React.FC<{ options: ToastOptions; onRemove: (id: number) => void }> = ({ options, onRemove }) => {
    ...
    const icons = { success: <SuccessIcon className="w-6 h-6 text-green-500"/>, error: <ErrorIcon className="w-6 h-6 text-orange-500"/>, info: <InfoIcon className="w-6 h-6 text-blue-500"/> };
