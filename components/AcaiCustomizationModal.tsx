...
const AcaiCustomizationModal: React.FC<AcaiCustomizationModalProps> = ({
    ...
    const { totalPrice, currentFreeCount, isLimitReached, isGlobalLimitReached } = useMemo(() => {
        ...
        selectedAddonIds.forEach(id => {
            const addon = availableAddons.find(a => String(a.id) === id);
            if (addon) {
        ...
        return {
            ...
            isGlobalLimitReached: initialItem.maxAddons !== undefined && 
                                 initialItem.maxAddons !== null && 
                                 (Number(initialItem.maxAddons) === 0 || selectedAddonIds.size >= Number(initialItem.maxAddons))
        };
    ...
    const handleAddonToggle = (addon: Addon) => {
        ...
        setSelectedAddonIds(prev => {
            ...
            const isSelected = newSet.has(idStr);
                            ...
                            <div className="mb-8">
                                <div className={`flex flex-col mb-5 p-5 rounded-3xl border-2 transition-all duration-500 ${isLimitReached ? 'bg-green-50 border-green-200 shadow-lg shadow-green-50' : 'bg-purple-50 border-purple-100'}`}>
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h4 className={`font-black text-sm uppercase tracking-tight flex items-center gap-2 ${isLimitReached || (initialItem.maxAddons && currentFreeCount >= initialItem.maxAddons) ? 'text-green-900' : 'text-purple-900'}`}>
                                                Itens Inclusos
                                                {(isLimitReached || (initialItem.maxAddons && currentFreeCount >= initialItem.maxAddons)) && <CheckCircle2 className="w-4 h-4 text-green-600" />}
                                            </h4>
                                            <p className={`text-[10px] font-bold uppercase tracking-widest ${isLimitReached || (initialItem.maxAddons && currentFreeCount >= initialItem.maxAddons) ? 'text-green-600' : 'text-purple-600'}`}>
                                                {isLimitReached || (initialItem.maxAddons && currentFreeCount >= initialItem.maxAddons) ? 'Seleção Completa!' : `Escolha até ${initialItem.maxAddons || freeAddonCountLimit} opções`}
                                            </p>
                                        </div>
                                        <div className="text-right">
                                            <div className={`text-2xl font-black tabular-nums ${isLimitReached || (initialItem.maxAddons && currentFreeCount >= initialItem.maxAddons) ? 'text-green-600' : 'text-purple-800'}`}>
                                                {currentFreeCount}<span className="text-purple-300 text-sm mx-1">/</span>{initialItem.maxAddons || freeAddonCountLimit}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 gap-2.5">
                                    {freePool.map(addon => {
                                        ...
                                        const isDisabled = !isSelected && (isLimitReached || isGlobalLimitReached);
                        ...
                        {paidPool.length > 0 && (
                            <div>
                                ...
                                <div className="grid grid-cols-1 gap-2.5">
                                    {paidPool.map(addon => {
                                        const isSelected = selectedAddonIds.has(String(addon.id));
                                        const isDisabled = !isSelected && isGlobalLimitReached;
                                        
                                        return (
                                            <label key={addon.id} className={`group flex items-center justify-between p-4 border-2 rounded-2xl transition-all duration-300 ${
                                                isDisabled ? 'opacity-40 bg-gray-50 cursor-not-allowed grayscale' : 'cursor-pointer hover:border-orange-200 hover:bg-orange-50/20'
                                            } ${isSelected ? 'bg-orange-50 border-orange-500 shadow-md ring-2 ring-orange-100' : 'border-gray-100 bg-white'}`}>
                    ...
                    >
                        {isAdding ? (
                            <>
                                ...
                                <span>{initialCartItem ? 'Atualizado com Sucesso!' : 'Adicionado com Sucesso!'}</span>
                            </>
                        ) : (
                            <>
                                <ShoppingBag className="w-5 h-5 group-hover:animate-bounce" />
                                <span>{initialCartItem ? 'Confirmar e Atualizar' : 'Confirmar e Adicionar'}</span>
                            </>
                        )}
                    </button>
                </div>
