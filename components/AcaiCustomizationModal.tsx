...
const AcaiCustomizationModal: React.FC<AcaiCustomizationModalProps> = ({
    ...
    const { totalPrice, currentFreeCount, isLimitReached } = useMemo(() => {
        ...
        return {
            totalPrice: basePrice + addonsTotal + optionsPrice,
            currentFreeCount: freeCount,
            isLimitReached: freeCount >= freeAddonCountLimit
        };
    }, [initialItem, selectedSize, selectedAddonIds, availableAddons, freeAddonCountLimit, selectedOptions]);
    ...
    const handleAddToCartClick = () => {
        ...
        const optionsNames = cartSelectedOptions.map(o => o.optionName).join(', ');
        
        const name = `${initialItem.name} ${selectedSize.name}`;
        
        const descriptionParts = [];
        if (optionsNames) descriptionParts.push(`Opções: ${optionsNames}`);
        ...
        const cartId = `acai-${initialItem.id}_size-${selectedSize.name}_addons-${addonIds}_options-${optionKey}`;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[120] flex justify-center items-center p-4" onClick={onClose} aria-modal="true" role="dialog" aria-labelledby="acai-modal-title">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="overflow-y-auto p-4 space-y-6 bg-gray-50 flex-grow">
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                        <h3 className="font-bold text-gray-800 mb-4 text-lg border-b pb-2">Personalize</h3>
                        
                        {initialItem.optionGroups?.map(group => {
                                    ...
                                    <div className="flex justify-between items-center mb-3">
                                        <h4 className="font-bold text-gray-700 flex items-center gap-2">
                                            ...
                                            {group.minSelections > 0 && (
                                                <span className="bg-red-100 text-red-600 text-[9px] px-2 py-0.5 rounded-full font-black uppercase tracking-tighter">Obrigatório</span>
                                            )}
                                        </h4>
                                                ...
                                                <label key={option.name} className={`flex items-center justify-between p-3 border rounded-xl transition-all cursor-pointer hover:bg-purple-50/20 ${isOptionSelected ? 'bg-purple-50 border-purple-400 ring-1 ring-purple-400' : 'border-gray-200'}`}>
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-5 h-5 rounded flex items-center justify-center transition-all ${
                                                            ...
                                                            {isOptionSelected && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                                                        </div>
                                                        <input 
                                                    ...
                                                    {option.price > 0 && (
                                                        <span className="text-xs font-black text-gray-500">+ R$ {option.price.toFixed(2)}</span>
                                                    )}
                                                </label>
                                            );
                                        })}
                ...
                <div className="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                    ...
                    <button 
                        ...
                        className={`w-full font-bold py-3.5 px-6 rounded-xl transition-all active:scale-[0.98] shadow-lg flex justify-center items-center gap-2 ${!isAllValid() ? 'bg-gray-300 text-gray-400 cursor-not-allowed' : (isAdding ? 'bg-green-600 text-white scale-105' : 'bg-purple-600 text-white hover:bg-purple-700 shadow-purple-200')}`}
                    >
                        <span>{isAdding ? 'Adicionado!' : (!isAllValid() ? 'Conclua as opções' : 'Adicionar ao Carrinho')}</span>
                        {isAdding ? (
