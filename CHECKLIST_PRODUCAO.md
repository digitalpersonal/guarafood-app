...
3. **Função de Criar Restaurante (Admin):**
   ...
   ```
4. **Função de Excluir Restaurante (Admin):**
   ```bash
...
## 5. Solução de Problemas de Banco de Dados (SQL Mestre - RLS)

Se você encontrar erros como `Failed to fetch` ou erro ao carregar o cardápio, rode este **SCRIPT MESTRE DE PERMISSÕES** no SQL Editor do Supabase:
...
-- 2. LIMPEZA: REMOVE TODAS AS POLÍTICAS ANTIGAS PARA EVITAR CONFLITOS
...
-- 3. CRIAR POLÍTICAS DE LEITURA PÚBLICA (RESOLVE O "FAILED TO FETCH")
...
-- 4. CRIAR POLÍTICAS DE ADMINISTRAÇÃO (PARA O LOJISTA)
...
Este script garante que você possa fazer upload de fotos dos pratos e que os usuários sejam criados corretamente.
...
-- Criação dos Buckets (Pastas) para Imagens
...
-- Políticas para LOGOS DE RESTAURANTES
...
-- =========================================================
-- 2. GATILHO AUTOMÁTICO DE USUÁRIOS (TRIGGERS)
-- =========================================================

-- Função que roda automaticamente quando um usuário é criado
...
## 7. Atualização de Estrutura (Migração Final)

Se o banco de dados for antigo, algumas colunas novas (Taxa de Entrega, Pix Manual) podem estar faltando. Rode este script para garantir que tudo está atualizado.
...
-- 2. Atualizar Tabela de Categorias do Cardápio (Ícones)
ALTER TABLE public.menu_categories ADD COLUMN IF NOT EXISTS icon_url text;

-- 3. Inserir Categorias Padrão de Restaurante (se estiver vazio)
...
INSERT INTO public.restaurant_categories (name)
SELECT 'Açaí' WHERE NOT EXISTS (SELECT 1 FROM public.restaurant_categories WHERE name = 'Açaí');
...
## 8. Correção de Permissões de Cadastro (Produtos e Categorias)

Se você tiver problemas para criar itens ou categorias, rode este script. Ele libera a escrita para os administradores nas tabelas certas.

```sql
BEGIN;

-- 1. Reforçar Políticas de PRODUTOS (menu_items)
DROP POLICY IF EXISTS "Merchant Manage Items" ON public.menu_items;
CREATE POLICY "Merchant Manage Items" 
...
-- 2. Reforçar Políticas de CATEGORIAS DO CARDÁPIO (menu_categories)
DROP POLICY IF EXISTS "Merchant Manage Categories" ON public.menu_categories;
...
-- 3. Reforçar Políticas de ADICIONAIS (addons)
DROP POLICY IF EXISTS "Merchant Manage Addons" ON public.addons;
CREATE POLICY "Merchant Manage Addons" 
ON public.addons 
FOR ALL 
USING (
  auth.uid() IN (
    SELECT id FROM profiles 
    WHERE restaurant_id = addons.restaurant_id 
    OR role = 'admin'
  )
);

-- 4. CORREÇÃO CRÍTICA: Permitir Admin gerenciar Categorias de Restaurante
DROP POLICY IF EXISTS "Admin Manage Restaurant Categories" ON public.restaurant_categories;
CREATE POLICY "Admin Manage Restaurant Categories" 
