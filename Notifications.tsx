...
const MenuManagement: React.FC<{ restaurantId?: number, onBack?: () => void }> = ({ restaurantId: propRestaurantId, onBack }) => {
    ...
    const [editingCategory, setEditingCategory] = useState<{ 
        ...
        newAvailableStartTime?: string;
        newAvailableEndTime?: string;
    } | null>(null);
    ...
    const handleEditCategory = (category: MenuCategory) => {
        setEditingCategory({ 
            ...
            newAvailableStartTime: category.availableStartTime || '',
            newAvailableEndTime: category.availableEndTime || ''
        });
    };
    ...
    const handleSaveCategoryChanges = async () => {
        ...
        const currentCategory = menuCategories.find(c => c.id === id);

        if (newName.trim() === '' || (
            newName === oldName && 
            newIconUrl === (currentCategory?.iconUrl || null) &&
            (newAvailableStartTime || '') === (currentCategory?.availableStartTime || '') &&
            (newAvailableEndTime || '') === (currentCategory?.availableEndTime || '')
        )) {
        ...
        try {
            await updateCategory(
                ...
                newName.trim(), 
                newIconUrl, 
                newAvailableStartTime?.trim() || null, 
                newAvailableEndTime?.trim() || null
            );
                                ...
                                <div className="flex justify-between items-center p-3 bg-gray-50 rounded-t-lg border-b group">
                                    <div className="flex items-center gap-2 flex-grow">
                                        ...
                                        {editingCategory?.id === category.id ? (
                                            <div className="flex flex-col w-full max-w-md gap-3 bg-white p-3 rounded-xl border border-orange-200 shadow-sm my-1">
                                                <div>
                                                    <label className="text-xs font-bold text-gray-700 block mb-1">Nome da Categoria</label>
                                                    <input 
                                                        type="text" 
                                                        ...
                                                        autoFocus
                                                        className="text-base font-bold p-1.5 border rounded-md w-full focus:ring-2 focus:ring-blue-500"
                                                        placeholder="Nome da Categoria"
                                                    />
                                                </div>
                                                <div>
                                                    <label className="text-xs font-bold text-gray-700 block mb-1">Ícone / Imagem (Opcional)</label>
                                                    <div className="flex items-center gap-2">
                                                        <input 
                                                            type="file" 
                                                            accept="image/*" 
                                                ...
                                                <div className="bg-blue-50/70 p-2.5 rounded-lg border border-blue-100">
                                                    <label className="text-xs font-extrabold text-blue-900 block mb-1 flex items-center gap-1">
                                                        <span>⏰</span> Horário de Venda da Categoria (Turno)
                                                    </label>
                                                    <p className="text-[11px] text-blue-700 mb-2">Se esta categoria só vende num horário específico (ex: Lanches à noite), informe abaixo. Deixe em branco para vender a qualquer hora.</p>
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <span className="text-[10px] text-gray-600 font-bold block">A partir das:</span>
                                                            <input 
                                                                type="time" 
                                                                ...
                                                                className="text-xs p-1.5 border rounded-md w-full bg-white font-medium"
                                                            />
                                                        </div>
                                                        <div>
                                                            <span className="text-[10px] text-gray-600 font-bold block">Até às:</span>
                                                            <input 
                                                                type="time" 
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                {(category.availableStartTime || category.availableEndTime) && (
                                                    <span className="text-xs font-bold text-blue-700 flex items-center gap-1 mt-0.5 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200 w-fit">
                                                        ⏰ Vendido {category.availableStartTime && category.availableEndTime ? `das ${category.availableStartTime} às ${category.availableEndTime}` : category.availableStartTime ? `a partir das ${category.availableStartTime}` : `até às ${category.availableEndTime}`}
                                                    </span>
                                                )}
                                            </div>
                                        )}
