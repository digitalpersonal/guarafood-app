...
import { isMenuItemAvailableByTime } from '../utils/restaurantUtils';
...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, allPizzas, allAddons, isOpen = true, categoryName, category }) => {
  ...
  const timeCheck = isMenuItemAvailableByTime(item, category);
  const isTimeAvailable = timeCheck.isAvailable;
  const canPurchase = isOpen && isAvailable && isTimeAvailable;
  ...
  const handleAddToCartClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!isTimeAvailable) {
        addToast({ message: timeCheck.message || "Este item não está disponível neste horário.", type: 'warning' });
        return;
    }
  ...
  return (
    <>
      <div className={`${containerClasses} rounded-lg overflow-hidden flex p-3 space-x-4 relative transition-all duration-300 group ${!canPurchase ? 'grayscale opacity-75' : ''} ${isAdding ? 'ring-2 ring-green-500 bg-green-50' : ''}`}>
          ...
          {/* Badge: Fora do Horário */}
          {!isTimeAvailable && isAvailable && (
            <div className="absolute top-0 right-0 bg-blue-600 text-white text-[10px] font-extrabold px-2 py-1 rounded-bl-lg z-10 flex items-center gap-1 shadow-sm">
               <span>⏰ {item.availableStartTime ? `A PARTIR DAS ${item.availableStartTime}` : 'INDISPONÍVEL AGORA'}</span>
            </div>
          )}
        ...
        <div className="flex-grow pt-4"> 
          ...
          {(item.availableStartTime || item.availableEndTime) && (
              <div className={`flex items-center gap-1 text-[10px] font-bold rounded-full px-2.5 py-0.5 w-fit my-1 border ${
                  isTimeAvailable
                  ...
                  <span>
                      {isTimeAvailable 
                          ? (item.availableStartTime && item.availableEndTime ? `Vendido das ${item.availableStartTime} às ${item.availableEndTime}` : item.availableStartTime ? `Vendido a partir das ${item.availableStartTime}` : `Vendido até às ${item.availableEndTime}`)
                          : (timeCheck.message || 'Fora do horário de vendas')
                      }
                  </span>
              </div>
          )}
