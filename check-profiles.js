...
supabase.from('profiles').select('restaurant_id').limit(1).then(({ error }) => console.log("restaurant_id error:", error));
supabase.from('profiles').select('restaurantId').limit(1).then(({ error }) => console.log("restaurantId error:", error));
